...
if __name__ == '__main__':
    ...
    for i in range(len(board)):
        temp_data = [col[i] for col in board if col[i] > 0]
        transfer_board.append(temp_data[::-1])

    bf_num = 0
    for item in moves:
        doll_num = 0
        if transfer_board[item-1]:
            doll_num = transfer_board[item-1].pop()
        else:
            continue

        if doll_num == bf_num:
            del(basket[len(basket)-1])
            bf_num = basket[len(basket)-1]
            answer+=2
        else:
